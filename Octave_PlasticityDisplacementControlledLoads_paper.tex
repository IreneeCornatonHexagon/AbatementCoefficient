...
\label{eq:RO}
\end{equation}
with, for 316L(N) stainless steel, $n_0 = 0.1125$ ($1/n_0 = 8.89$) and $C_0 = 1.198$; at the benchmark temperature of \SI{550}{\celsius}, $R_{p0.2} \approx \SI{106}{MPa}$, so that the stress scale $C_0R_{p0.2}$ is about \SI{131.8}{MPa}. The temperature dependence therefore enters through $R_{p0.2}(T)$, whereas $C_0$ and $n_0$ retain the same normalised curve shape at all temperatures --- i.e., no change of hardening \emph{shape} with temperature is assumed. A sensitivity study reported in the presentation, using an alternative parameter set ($C_0=1.233$, $n_0=0.130$, i.e. $1/n_0=7.69$, $C_0R_{p0.2}\approx\SI{135.6}{MPa}$) calibrated on the same \SI{550}{\celsius} tensile data, produces a stress--strain curve only marginally softer than the reference one (e.g. about \SI{2}{MPa} lower at $\varepsilon_t=2\%$), confirming that the choice of hardening parameters is not the dominant source of discrepancy discussed in Section~\ref{sec:discussion}.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.73\textwidth]{figures/rm_graphical_definition_overview.png}
\caption{Slide-level synthesis of the RCC-MRx elastic-follow-up construction: the elastic and elasto-plastic curves, the local resilience $t=OM/MN$, the global resilience $T$, and the slope associated with the spring-effect line are brought together in one graphical definition of $r_M$.}
...
This appendix reproduces, for completeness, the mathematical derivation of the elastic follow-up factor $r_M$ from the energy balance of a purely elastic finite-element analysis, following R.~Roche's original method \cite{Roche1987} as re-derived in an internal technical note accompanying the source presentation \cite{RochePipeStressNote}. This is the rigorous justification of Eqs.~\eqref{eq:rM}--\eqref{eq:Tglobal} used in Section~\ref{sec:rccmrx}. Figures~\ref{fig:roche-graphical}, \ref{fig:roche-calculation-sheet} and \ref{fig:roche-note-pages} reproduce the graphical and documentary material from which the algebraic steps below were reconstructed.
...
\caption{Numerical worksheet from the source presentation reproducing Roche's two-section example: local elastic and plastic strains are evaluated separately in the weak and strong sections, then combined to obtain $T=2.006$ and the local elastic follow-up levels reported in the surrounding discussion.}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{figures/roche_theorem_note_pages.png}
\caption{Composite of the original note pages used to reconstruct Roche's derivation in this appendix: virtual-work development, thermal-strain extension, energy-balance definition of $T$, and final derivation of the isotropic relation $r_M=\tfrac{2(1+\nu)}{3}(T/t-1)$.}
\label{fig:roche-note-pages}
\end{figure}
